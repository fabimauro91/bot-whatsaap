...
// Función para el manejo de mensajes con el agente inteligente
async function handleMessageWithAgent(message, instanceId, instanceInfo) {
    if (message.fromMe) return;

    const userId = message.from;
    const messageContent = message.body;
    console.log(`Mensaje recibido en instancia ${instanceInfo.id} de ${userId}: ${messageContent}`);

    try {
        // Obtener cliente de la instancia
        const instanceData = instances.get(instanceId);
        if (!instanceData || !instanceData.client) {
            throw new Error('Cliente de WhatsApp no disponible');
        }
        
        // Verificar si ya existe un agente para esta instancia
        if (!instanceData.agente) {
            console.log(`Creando nuevo agente para instancia ${instanceId}`);
            
            // Crear instancia de Agente y cargar productos
            instanceData.agente = new Agente(instanceData.client);
            
            // Obtener código de sucursal para cargar productos
            const codigoSucursal = instanceData.codigo_sucursal;
            console.log(`Cargando productos para sucursal ${codigoSucursal}`);
            
            // Cargar productos
            try {
                // Primero obtener datos de la sucursal desde la BD
                const sucursal = await Sucursal.findOne({
                    where: { codigo_sucursal: codigoSucursal }
                });
                
                if (!sucursal) {
                    console.error(`❌ Sucursal con código ${codigoSucursal} no encontrada en la base de datos`);
                    throw new Error(`Sucursal no encontrada: ${codigoSucursal}`);
                }
                
                console.log(`✓ Datos de sucursal obtenidos: ${sucursal.nombre_sucursal}, ID Usuario: ${sucursal.id_users || 'No disponible'}`);
                
                // Ahora usamos el id_users de la sucursal para cargar los productos
                await instanceData.agente.cargarProductosDesdeAPI(sucursal.id_users);
                console.log(`✅ Productos cargados correctamente para sucursal ${codigoSucursal}`);
            } catch (errorProductos) {
                console.error(`❌ Error al cargar productos para sucursal ${codigoSucursal}:`, errorProductos);
                // Intentar cargar desde JSON como respaldo
                instanceData.agente.cargarProductosDesdeJSON();
            }
        }
        
        // Verificar si hay un procesamiento de compra en curso
        const agente = instanceData.agente;
        
        // Primero verificar si es una continuación de un proceso de compra
        const esProcesoDePedido = await agente.procesarDatosCliente(message);
        if (esProcesoDePedido) {
            console.log(`Procesando datos de pedido para usuario ${userId}`);
            return true;
        }
        
        // Si el mensaje contiene explícitamente "quiero comprar" o similar, procesarlo directamente
        const mensajeLower = messageContent.toLowerCase();
        if (mensajeLower.includes("quiero comprar") || 
            mensajeLower.includes("comprar este") || 
            mensajeLower.includes("adquirir este") ||
            mensajeLower.includes("me lo llevo") ||
            mensajeLower.includes("lo compro")) {
            
            try {
                console.log("Detectada intención de compra explícita");
                
                // Buscar el producto más reciente mencionado en la conversación
                let ultimoProducto = null;
                const productosEnCache = agente.productosCache;
                
                if (productosEnCache && productosEnCache.length > 0) {
                    // Buscar en el historial de conversación el último producto mencionado
                    const conversacion = agente.conversacionCache;
                    
                    for (const producto of productosEnCache) {
                        if (conversacion.includes(producto.nombre_producto)) {
                            ultimoProducto = producto;
                            // Seguimos buscando para encontrar el último mencionado
                        }
                    }
                    
                    if (ultimoProducto) {
                        console.log(`Producto identificado para compra: ${ultimoProducto.nombre_producto}`);
                        await agente.iniciarProcesoCompra(message, ultimoProducto);
                        return true;
                    }
                }
                
                // Si no se encuentra un producto específico, caer en el procesamiento normal
                console.log("No se pudo identificar un producto específico, continuando con proceso normal");
            } catch (compraError) {
                console.error("Error al procesar compra directa:", compraError);
                // Continuar con el procesamiento normal
            }
        }
        
        // Intenta usar Gemini, pero maneja el error de cuota excedida
        try {
            // Primero verificar si está preguntando por un producto específico
            const esConsultaProducto = await agente.consultarGeminiConProductos(message, genAI);
            if (esConsultaProducto) {
                // Si ya se procesó como consulta de producto, terminamos
                return true;
            }
            
            // Verificar si es solicitud de compra
            const esSolicitudCompra = await agente.procesarSolicitudCompra(message, genAI);
            if (esSolicitudCompra) {
                // Si ya se procesó como solicitud de compra, terminamos
                return true;
            }
            
            // Si no es consulta de producto ni solicitud de compra, usar consulta general
            const respuesta = await agente.consultarGemini(messageContent, genAI);
            await agente.enviarMensaje(message, respuesta);
            
            console.log(`Respuesta enviada en instancia ${instanceInfo.id}: "${respuesta.substring(0, 60)}..."`);
        } catch (geminiError) {
            // Verificar si es un error de cuota excedida
            if (geminiError.message && geminiError.message.includes('429') && 
                geminiError.message.includes('quota')) {
                
                console.log("Detectado error de cuota de Gemini, usando respuesta de respaldo");
                
                // Respuesta de emergencia cuando Gemini no está disponible
                let respuestaEmergencia = "Lo siento, nuestro sistema de asistencia inteligente está experimentando alta demanda en este momento. ";
                
                // Si tenemos productos en caché, ofrecer opciones básicas
                if (agente.productosCache && agente.productosCache.length > 0) {
                    const categoriasDisponibles = [...new Set(agente.productosCache.map(p => p.id_categoria))];
                    
                    respuestaEmergencia += "Mientras tanto, puedo ayudarte con información básica sobre nuestros productos. ";
                    respuestaEmergencia += "\n\nCategorías disponibles:\n";
                    
                    // Mostrar hasta 5 categorías
                    const categoriasAMostrar = categoriasDisponibles.slice(0, 5);
                    categoriasAMostrar.forEach(cat => {
                        respuestaEmergencia += `- ${cat}\n`;
                    });
                    
                    if (categoriasDisponibles.length > 5) {
                        respuestaEmergencia += "- Y más...\n";
                    }
                    
                    respuestaEmergencia += "\nPuedes preguntar por productos específicos o categorías.";
                } else {
                    respuestaEmergencia += "Por favor, intenta nuevamente en unos minutos o contáctanos por otro medio si es urgente.";
                }
                
                await agente.enviarMensaje(message, respuestaEmergencia);
            } else {
                // Otro tipo de error
                console.error("Error en procesamiento con Gemini:", geminiError);
                await agente.enviarMensaje(message, "Lo siento, estoy experimentando dificultades técnicas. ¿Puedes intentar nuevamente con una consulta más simple?");
            }
        }
        
        return true;
    } catch (error) {
        console.error(`Error procesando mensaje en instancia ${instanceInfo.id}:`, error);
        
        // Mensaje de error personalizado
        await message.reply('Lo siento, en este momento estoy experimentando dificultades técnicas. Por favor, intenta nuevamente en unos minutos.');
...
async function createWhatsAppInstance(codigo_sucursal) {
    ...
    try {
        ...
        // Manejar mensajes con el agente
        client.on('message', async (message) => {
            if (message.fromMe) return;
            await handleMessageWithAgent(message, instanceId, instance);
        });
        ...
        if (!client.listenerCount('message')) {
            ...
            // Reconfigurar el evento de mensajes con el agente
            client.on('message', async (message) => {
                if (message.fromMe) return;
                await handleMessageWithAgent(message, instanceId, instance);
            });
