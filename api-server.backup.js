    ...
    try {
        ...
        client.on('ready', async () => {
            try {
                ...
                try {
                    ...
                    if (!state) {
                        console.log('❌ No se pudo obtener el estado - Conexión parcial');
                        // Intentar una reconexión completa
                        await client.resetState();
                        await new Promise(resolve => setTimeout(resolve, 2000));
                        await client.initialize();
                        console.log('✓ Reconexión completa realizada');
                    } else {
                        console.log(`✓ Estado de WhatsApp verificado: ${state}`);
                        console.log('✓ Bot totalmente funcional');
                    }
                } catch (stateError) {
                    console.error('Error al verificar estado:', stateError);
                    console.log('❌ Bot puede tener funcionalidad limitada');
                }
        ...
        client.on('change_state', async (state) => {
            ...
            if (state === 'TIMEOUT' || state === 'CONFLICT' || state === 'UNLAUNCHED') {
                ...
                // Solo intentar reconectar si no está ya desconectado
                if (connectionStatus !== 'disconnected') {
                    ...
                    try {
                        ...
                        console.log('❌ La reconexión automática falló');
                    }
                }
            }
        });
        ...
        // Forzar una reconexión completa para asegurar que todo funciona correctamente
        console.log('Forzando reconexión completa para garantizar estabilidad...');
        try {
            await client.resetState();
            await new Promise(resolve => setTimeout(resolve, 3000));
            await client.initialize();
            ...
            console.error('Error durante la reconexión forzada:', resetError.message);
        }
        ...
        try {
            ...
            // Intentar enviar un mensaje de ping a nosotros mismos para verificar la conexión
            let isConnected = false;
            try {
                // Intenta obtener el estado
                const state = await client.getState();
                console.log(`Estado inicial: ${state || 'desconocido'}`);
                
                // Si hay estado pero no está conectado, forzar reconexión
                ...
                // Verificar otra vez después de reintentar
                ...
                if (isConnected) {
                    ...
                    console.log('⚠️ No se pudo establecer conexión activa');
                }
            } catch (verifyError) {
